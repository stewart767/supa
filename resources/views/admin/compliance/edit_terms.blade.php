<x-app-layout title="Edit Terms & Conditions Draft">
    <x-slot name="header">Edit Terms & Conditions Draft (v{{ $terms->version }})</x-slot>

    <div class="max-w-4xl mx-auto bg-white p-8 rounded-3xl border border-slate-200 shadow-xl space-y-6">
        <div class="flex justify-between items-center border-b border-slate-100 pb-4">
            <div class="space-y-1">
                <h3 class="text-lg font-extrabold text-slate-900">Terms & Conditions Revision Form</h3>
        ...
        <form action="{{ route('admin.compliance.terms.update', $terms->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 text-xs font-semibold" x-data="{ docType: '{{ $terms->file_path ? 'file' : ($terms->content ? 'text' : 'file') }}', fileName: '' }">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="version" value="{{ old('version', $terms->version) }}" required class="w-full px-4 py-3 rounded-xl border border-slate-300 bg-slate-50 text-slate-900 outline-none focus:ring-2 focus:ring-amber-500">
                ...
                <div>
                    ...
                    <select name="language" required class="w-full px-4 py-3 rounded-xl border border-slate-300 bg-slate-50 text-slate-900 outline-none focus:ring-2 focus:ring-amber-500">
                        <option value="en" {{ old('language', $terms->language) === 'en' ? 'selected' : '' }}>English (en)</option>
                        <option value="sw" {{ old('language', $terms->language) === 'sw' ? 'selected' : '' }}>Swahili (sw)</option>
                    </select>
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $terms->title) }}" required class="w-full px-4 py-3 rounded-xl border border-slate-300 bg-slate-50 text-slate-900 outline-none focus:ring-2 focus:ring-amber-500">
                ...
                <div x-show="docType === 'file'" x-transition class="space-y-2">
                    @if($terms->file_path)
                        <div class="p-3 bg-blue-50/70 border border-blue-200/50 text-blue-900 rounded-xl mb-3 flex justify-between items-center">
                            <span class="text-[11px] font-bold">📄 Current File: {{ basename($terms->file_path) }}</span>
                            <a href="{{ asset('storage/' . $terms->file_path) }}" target="_blank" class="underline hover:text-blue-950 font-black">View File</a>
                        </div>
                    @endif
